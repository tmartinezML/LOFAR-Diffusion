...
def get_bdsf_metrics_plots(img_dir, save=True,
    ...
    # Get metrics dict
    gen_distr_dict_list = [
        get_bdsf_distributions(d) for d in img_dir
    ]
    if train_path is not None:
        lofar_distr_dict = get_bdsf_distributions(train_path)
    ...
    if save_fig:
        fig.savefig(
            out_dir / f"{fig_name}_{bdsf_metrics_plot}.pdf"
        )
...
def get_metric_dict(
    img_path,
    parent=paths.ANALYSIS_PARENT, force=False, save=True
):
    ...
    assert any(dicts_path.glob('*.pkl')), \
        f"No pickle files found in {dicts_path}"
...
def load_bdsf_metric_dict(bdsf_pkl_dir):
    ...
    out_dict = {k: elements_from_dict(bdsf_dicts, k)
                for k in tqdm(bdsf_dicts[0].keys())}
    return out_dict
